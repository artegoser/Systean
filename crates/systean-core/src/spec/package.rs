...
pub fn compile_sources(
...
) -> Result<Environment, Vec<PackageError>> {
    let mut parsed = Vec::new();
    let mut errors = Vec::new();
    for (source_name, text) in sources {
        match parse_specification(&text) {
            Ok(specification) => parsed.push(SourceSpecification::new(source_name, specification)),
            Err(parse_errors) => errors.extend(
                parse_errors
                    .into_iter()
                    .map(|error| PackageError::Parse { source: source_name.clone(), error }),
            ),
        }
    }
    if !errors.is_empty() {
    ...
    compile_specifications(&parsed)
        .map_err(|errors| errors.into_iter().map(PackageError::Compile).collect())
}
