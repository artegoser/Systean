...
fn phonology() -> PhonologyConfig {
    PhonologyConfig::load(
        repo_path("language/alphabet.toml"),
        repo_path("language/phonology.toml"),
    )
    .unwrap()
}
...
fn manual_root_checker_rejects_collisions_and_only_warns_on_similarity() {
    let phonology = phonology();
    let inventory = RootInventory::load_dictionary(repo_path("language/dictionary.toml")).unwrap();
...
fn existing_dictionary_roots_are_phonologically_valid_and_pronunciation_unique() {
    let phonology = phonology();
    let inventory = RootInventory::load_dictionary(repo_path("language/dictionary.toml")).unwrap();
    let by_pronunciation = systean_core::phonology::roots_by_pronunciation(&phonology, &inventory);
