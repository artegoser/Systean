use systean_core::semantics::{CheckError, Checker, Term, Type};
...
fn chumsky_parser_handles_function_and_generic_types() {
    assert_eq!(
        ...
        Type::Function {
            parameters: vec![systean_core::semantics::FunctionParameter {
...
fn alpha_equivalent_binders_have_one_canonical_term() {
    ...
    assert_ne!(x, person);
    assert_eq!(systean_core::semantics::canonicalize(&x), systean_core::semantics::canonicalize(&person));
}
...
fn explainer_reports_types_roles_and_definition_origins() {
    ...
    let rendered = systean_core::semantics::Explainer::new(&environment).explain(&term).unwrap().render();
